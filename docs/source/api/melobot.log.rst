...
在 3.2.0 版本，日志系统进行了重构，现在推荐通过“域”规则设置和获取日志器。依据“域”来获取日志代理对象，可使用 :func:`.get_logger`，或使用上下文变量 :data:`~melobot.log.logger` 获取。日志代理对象是通用日志器的子类，它将会内含一个有效的日志器或单纯包含空，包含空时自动丢弃所有日志记录
