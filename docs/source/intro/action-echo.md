...
当直接使用行为方法时，默认是尽快完成的。即不等待也不关心是否成功完成了行为。例如在 OneBot 协议支持中：
...
# 等待句柄，即等待输出源通知行为已完成
# 获取返回值，即响应结果，这在 melobot 中一般称为回应对象
echo = await handle
...
因此，如果只是想要保证操作有序性（即等待操作完成），实际上非常简单：
...
```python
# 访问需要的响应数据（data 字段与 OneBot 协议规定的数据结构一致）
...
句柄的本质是将操作和等待解耦。请发散思维来使用它：例如安排一批操作，后续再集中等待，实现并发操作。
