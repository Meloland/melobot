...
通过 API 参考文档，可以发现：行为操作函数，有一个共有参数 `wait`。当不指定这个 wait 参数，或设置 `wait=False` 时，行为操作函数在将行为传递给 OneBot 实现程序后，就**立即返回**。即使这个行为**还没有被 OneBot 实现程序“完成”**（即提交给 qq 服务器）。

```python
"""
这里也可以是 reply, send_reply, finish, reply_finish, send_forward 等一系列行为操作函数 
"""
...
响应对象的 {attr}`~.ResponseEvent.data` 属性是一个字典。不同的行为操作函数，会有不同的返回数据。返回数据和行为操作函数的对照表，参考 [OneBot 标准](https://github.com/botuniverse/onebot-11/blob/master/api/public.md) 中的响应数据。
...
```{admonition} 提示
:class: tip
不需要返回数据时，不建议等待响应（`wait=True`）。等待响应总是需要更多时间，大量使用会降低并发性能。
...
某些情况下，我们可能不希望行为对象构建完成后，就被直接发送。行为操作函数可以使用 `auto` 参数干预这个过程。`auto=False` 时，行为操作函数产生行为对象后，将会立即返回。
...
和之前提到的自定义消息内容一样，如果需要发送 OneBot 标准中不存在，但 OneBot 实现程序支持的行为，使用 {func}`.make_action` 构造行为对象：
