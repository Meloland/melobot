...
消息内容，由于有文本、语音、图片等多种类型。因此需要统一的数据结构来表示。在 OneBot 标准中，主要有两种格式，来表示消息内容这种数据结构：字符串（string）和数组（array）。

在 melobot 中，你不需要手动处理消息内容的数据结构，**虽然无需手动处理，但是了解消息内容的数据结构，有助于你使用 melobot 提供的相关方法**。
