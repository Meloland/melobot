...
每次读取 `mb_handle.event` 都是最新的事件，但是坏处是依然没有精确的类型注解。
...
`legacy_session` 参数只有文本事件的“绑定函数”拥有。其他“绑定函数”一般只会有 `rule` 参数。这是其他“绑定函数”对应处理的事件类型，一般而言搞“传统会话”没有太大意义。因此必须提供具体的规则。会话规则的自定义、调整会话创建的时机等属于高级特性与用法，将会在后面的教程中慢慢揭晓。
