...
def logger_patch(logger: Any, log_level: LogLevel = LogLevel.INFO) -> None:
    ...
    setattr(
        logger,
        BotLogger._check_level.__name__,
        types.MethodType(BotLogger._check_level, logger),
    )
    setattr(logger, BotLogger.exc.__name__, types.MethodType(BotLogger.exc, logger))
    setattr(logger, BotLogger.obj.__name__, types.MethodType(BotLogger.obj, logger))


# def log(msg: str, level: LogLevel = LogLevel.DEBUG, before: bool = False):
#     logger = logging.getLogger("abc")

#     def deco_func(func: Callable[P, T]) -> Callable[P, T]:
#         log_meth = partial(getattr(logger, level._name_.lower()), msg)
...
#     def deco_func(func: Callable[P, T]) -> Callable[P, T]:
#         log_meth = partial(getattr(logger, BotLogger.exc.__name__), msg)
