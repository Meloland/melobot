...
from melobot.ctx import BotCtx
from melobot.exceptions import PluginAutoGenError
from melobot.plugin.base import PluginPlanner
from melobot.plugin.ipc import AsyncShare, SyncShare


def main(args: Namespace) -> None:
    if not len(args.plugin_dirs):
        print("未提供插件目录参数")
        return

    p_dirs = set(Path(p_dir) for p_dir in args.plugin_dirs)
    PluginInitHelper.run_init(*p_dirs, load_depth=args.depth)
    print(f"已完成 {len(args.plugin_dirs)} 个插件的初始化：")
    for p_dir in p_dirs:
        print(f"  - {p_dir}")
...
class PluginInitHelper:
    _BASE_INIT_PY_STR = (
        Path(__file__).parent.joinpath("__init__.template.py").read_text(encoding="utf-8")
    )
    ...
    def _get_init_pyi_str(
        ...
        return "\n\n".join(filter(lambda x: x != "", (imp_lines, gets_str, vars_str))) + "\n"

    @classmethod
    def run_init(cls, *plugin_dirs: str | PathLike[str], load_depth: int = 1) -> None:
