...
class BotLinker:
    ...
    async def start(self) -> None:
        ...
        await self.ws.recv()
        BOT_STORE.logger.info('与 cq 成功建立 websocket 连接')
    ...
    async def close(self) -> None:
        ...
        """
        await self.ws.close()
        BOT_STORE.logger.info('与 cq 的连接已正常关闭')
        ...
        try:
            while True:
                try:
                    ...
                    await aio.sleep(BOT_STORE.config.cooldown_time)
                ...
                try:
                    ...
                    if self.event_q.full():
                        try:
                            ...
                            pass
                        BOT_STORE.logger.warning('事件队列已满！短时间内可能无法响应事件')
                        continue
    ...
    async def recv_prior_event(self, event: BotEvent) -> None:
        ...
        try:
            ...
            if self.prior_event_q.full():
                BOT_STORE.logger.warning('优先事件队列已满！短时间内可能无法响应更多优先事件')
            await self.prior_event_q.put(event)
        except aio.CancelledError:
            BOT_STORE.logger.debug('优先事件放置因超时被取消')
